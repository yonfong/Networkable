        ...
        return request(target) { (result) in
            ...
            case let .success(response):
                do {
                    ...
                    if let verifyResponse = mapedResponse as? ResponseVerifiable, let verifyResult = verifyResponse.verifyResult {
                        completionHandler?(.failure(.verifyError(code: verifyResult.errorCode, message: verifyResult.errorMesasge)))
                    } else {
